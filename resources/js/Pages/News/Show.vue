      ...
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 sm:pt-16" :class="locale === 'ar' ? 'text-right' : 'text-left'">
        ...
        <header class="space-y-6 mb-10">
          ...
          <div class="flex flex-wrap items-center gap-2 sm:gap-3 text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-mono" :dir="locale === 'ar' ? 'rtl' : 'ltr'">
            ...
            <span v-if="newsItem.sentiment === 'Bullish'" class="px-2 py-1.5 rounded-md bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20 font-bold">🟢 Bullish</span>
            <span v-else-if="newsItem.sentiment === 'Bearish'" class="px-2 py-1.5 rounded-md bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20 font-bold">🔴 Bearish</span>
            <span v-else-if="newsItem.sentiment === 'Neutral'" class="px-2 py-1.5 rounded-md bg-slate-500/10 text-slate-600 dark:text-slate-400 border border-slate-500/20 font-bold">⚪ Neutral</span>
            ...
            <span v-if="newsItem.impact_score" class="px-2 py-1.5 rounded-md bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 font-bold" title="Market Impact Score">⚡ {{ newsItem.impact_score }}/10</span>
          </div>
        ...
        <div class="mt-8">
          <h3 v-if="newsItem.ai_processed" class="text-xs font-bold text-slate-400 mb-6 uppercase tracking-widest border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center gap-2" :dir="locale === 'ar' ? 'rtl' : 'ltr'">
            <span>📰</span> <span>Full Article (AI Rewritten)</span>
          </h3>
        ...
        <div class="mt-16 p-5 sm:p-6 bg-slate-100 dark:bg-[#151e32] border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col sm:flex-row gap-5 items-start shadow-sm" :dir="locale === 'ar' ? 'rtl' : 'ltr'">
          ...
          <div>
            ...
            <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-mono">
              This content has been automatically generated and rewritten by the platform AI engine for informational purposes only, and does not constitute financial advice.
            </p>
          </div>
        </div>
